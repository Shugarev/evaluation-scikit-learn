...
class ParserArgs:
    ...
    def __init__(self, argv: Dict = None):
        ...
        for arg in ['--task', '--algorithm_name', '--model_name', '--input', '--input2', '--algorithm_config'
            , '--output', '--threshold', '--encode_config', '--encoded_path']:
    ...
    def parse_args_from_comand_line(self):
        ...
        Params.ANALYZER_PATH = Params.OUTPUT_PATH.split('.')[0] + "-analyzer.csv" if args.output else None
    ...
    def get_algorithm_params(self, config: ConfigParser) -> Dict:
        algorithm_params = {}
        if config:
    ...
    def get_params_range(self, config: ConfigParser) -> Dict:
